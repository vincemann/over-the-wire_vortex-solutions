 ...
 */
static
void MD5Update(MD5Context *pCtx, const unsigned char *input, unsigned int len){
    struct Context *ctx = (struct Context *)pCtx;
    uint32 t;
    ...
    if ( t ) {
        unsigned char *ctxInput = (unsigned char *)ctx->in + t;

        t = 64-t;
        if (len < t) {
            memcpy(ctxInput, input, len);
            return;
        }
        memcpy(ctxInput, input, t);
...
static unsigned char checkPassword(char* pw, int len){
...
//        printf("test\n");
    MD5Init(&md5Context);
    MD5Update(&md5Context, pw, len);
    MD5Final(hash, &md5Context);

    if(memcmp(hash, "\x15\x5f\xb9\x5d\x04\x28\x7b\x75\x7c\x99\x6d\x77\xb5\xea\x51\xf7", 16) == 0){
        printf("You got the right password, congrats!\n");
        printf("The raw password is: %s\n",pw);
        char printablePw[6];
        memcpy(printablePw,pw,5);
        printablePw[5] = '\0';
...
//        printf("The password is: %s",pw);
//        usleep(500);
//        printf("Incorrect password\n");
    }
}

static void checkPasswords(char* pws){
    int i;
    char* currPw = malloc(sizeof(char)* 5);
    for(i =0 ; i< strlen(pws); i+=5){
        memcpy(currPw,pws+i,5);
        i+=1;
        checkPassword(currPw,5);
    }
}
...
{
    generate(5);
    // will result in pw 'rlTf6'
    // execute /vortex/vortex5 and enter pw
    // -> *uy5qDRb2
}
